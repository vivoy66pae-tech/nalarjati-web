...
// Convert match to actual instant (UTC). For WIB display, add 7h to UTC components.
function matchToWIB(m) {
  const tz = VENUES[m.venue].tz;
  const [y, mo, d] = m.date.split("-").map(Number);
  const [hh, mm] = m.time.split(":").map(Number);
  // utc = actual UTC millis for the match
  const utc = Date.UTC(y, mo - 1, d, hh, mm) - tz * 3600 * 1000;
  return new Date(utc);
}

// Format a Date as WIB time string "HH:MM" and detect if day shifted
function dateToWIBTime(d) {
  let h = d.getUTCHours() + 7;
  let dayShift = 0;
  if (h >= 24) { h -= 24; dayShift = 1; }
  else if (h < 0) { h += 24; dayShift = -1; }
  const m = d.getUTCMinutes();
  return { time: `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`, dayShift };
}
...
// Get matches happening in the next 24 hours from now
function getNext24hMatches() {
  const now = new Date();
  const tomorrow = new Date(now.getTime() + 24 * 3600 * 1000);
  return MATCHES
    .filter(m => {
      const t = matchToWIB(m);
      return t >= now && t < tomorrow;
    })
    .sort((a, b) => matchToWIB(a) - matchToWIB(b));
}

// Get matches happening 24-48h from now
function getTomorrowMatches() {
  ...
  return MATCHES
    .filter(m => {
      const t = matchToWIB(m);
      return t >= new Date(from.getTime() + 24 * 3600 * 1000) && t < to;
    })
    .sort((a, b) => matchToWIB(a) - matchToWIB(b));
}
